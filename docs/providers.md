...
Pi is a process-backed provider. Paseo requires the user to have the `pi` binary installed and talks to it through `pi --mode rpc`; the server package does not embed Pi's SDK/runtime packages. Pi `v0.84.4` or newer is required for the `clear_queue` RPC used to discard unread steering and follow-up messages during interruption. Paseo does not enforce this version floor, so older Pi binaries remain usable but may deliver unread native queue entries on a later turn.
...
A steering adapter also owes its interrupt: stopping a turn must discard the steers the provider has not read yet, or one of them resumes the turn the user just stopped. Codex clears pending input when it aborts a turn; Claude cancels the SDK messages it queued before calling `query.interrupt()`. With Pi `v0.84.4` or newer, Paseo calls `clear_queue` before `abort` and discards the returned steering and follow-up entries. Older Pi binaries reject `clear_queue`, and cleanup failures still allow `abort` to proceed, so unread steers can resume on the next turn until Pi is updated or the cleanup succeeds.
